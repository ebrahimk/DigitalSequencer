...
/*static const uint8_t    color_mappings[13][3] = {
                      {0,0,0},
                      {255,0,0}, //Red
                      {255,125,0}, //Orange
                      {255,255,0}, //Yellow
...
static uint8_t com_pins[5] ={31,33,35,37,39};
      ...
      if( debounce_count[current][j] < MAX_DEBOUNCE)
      {
        ...
        if( debounce_count[current][j] == MAX_DEBOUNCE )
        {
          ...
          //Transmit code on serial3 
           ...
           if((value & B00000001) == B00000001){
                 digitalWrite(com_pins[0], HIGH);
                 Serial.println("1");
           }
           else {
                 digitalWrite(com_pins[0], LOW);
                           Serial.println("0");
           }
                 
          if((value & B00000010) == B00000010){
                 digitalWrite(com_pins[1], HIGH);
              Serial.println("1");
          }
           else {
                          Serial.println("0");
                 digitalWrite(com_pins[1], LOW);

           }
                 
          if((value & B00000100) == B00000100){
                 digitalWrite(com_pins[2], HIGH);
                 Serial.println("1");
          }
           else {
                          Serial.println("0");
                          digitalWrite(com_pins[2], LOW);
           }

          if((value & B00001000) == B00001000){
                 digitalWrite(com_pins[3], HIGH);
                 Serial.println("1");
          }
           else {
                          Serial.println("0");
                          digitalWrite(com_pins[3], LOW);
           }  
           digitalWrite(com_pins[4], HIGH);     //Communication bit, signifies that a signal is sending
           Serial.println("1");
      ...
      if( debounce_count[current][j] > 0)
      {
        ...
        if( debounce_count[current][j] == 0 )
        {
          ...
          if((value & B00001000) == B00001000){
           ...
           digitalWrite(com_pins[4], LOW);     //Communication bit, signifies that a signal is off 
                      Serial.println("0"); 
            */
